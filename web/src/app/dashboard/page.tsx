...
const loadOrganizerEvents = async () => {
    ...
    try {
        ...
        // 1. Get total events
        // Note: Optimally we would have `getEventsByOrganizer` on-chain, but for now we filter locally
        const totalEventsBigInt = await contract.getTotalEvents();
        const totalEvents = Number(totalEventsBigInt);
        ...
        for (let i = totalEvents - 1; i >= 0; i--) {
            try {
                ...
                if (organizer.toLowerCase() === address.toLowerCase()) {
                    ...
                    // ModuPassKRNL doesn't store "max_attendees" in the struct (it's in the args but not the state struct showed in previous files)
                    // Wait, `ModuPassKRNL.sol` state: `struct Event { id, name, organizer, createdAt, isActive }`
                    // It does NOT store maxAttendees on chain in the basic struct?
                    // I will default max_attendees to 100 if not found, or maybe I missed it.
                    // Let's assume 100 for now or fetch if possible.
